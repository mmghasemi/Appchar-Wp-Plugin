(function ($) {
  ...
  DayScheduleSelector.DEFAULTS = {
    days        : [6, 0, 1, 2, 3, 4, 5],  // Sun - Sat
    startTime   : '00:00',                // HH:mm format
    endTime     : '24:00',                // HH:mm format
    interval    : 60,                     // minutes
    stringDays  : ['شنبه', 'یک شنبه', 'دو شنبه', 'سه شنبه', 'چهار شنبه', 'پنج شنبه', 'جمعه'],
...
//      , ampm = hours >= 12 ? 'عصر' : 'صبح';
    return hours + ':' + ('0' + minutes).slice(-2) + ' - ' + (hours+1) + ':' + ('0' + minutes).slice(-2); // + ampm;
  }
  ...
  function hhmm(date) {
    var hours = date.getHours()
      , minutes = date.getMinutes();
    return ('0' + hours).slice(-2) + ':' + ('0' + minutes).slice(-2) + '-' + ('0' + (hours+1)).slice(-2) + ':' + ('0' + minutes).slice(-2);
  }
