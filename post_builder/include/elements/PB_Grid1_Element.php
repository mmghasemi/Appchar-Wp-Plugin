 ...
 * Time: 10:26 AM
 */
class PB_Grid1_Element extends PB_Element_Class implements PB_Element_Interface{
    ...
    public function __construct(){
        $this->type = 'grid1';
        $this->dashicon = 'dashicons-format-image';
        $this->title = __('full wide grid element','appchar');
        $this->values = '';
    ...
    {
        $items = array();
        foreach ($_POST['grid1_'.$index.'_image'] as $key=>$image){
            $items[]=array(
                'image'=> $image,
                'link_type'=>$_POST['grid1_'.$index.'_link_type'][$key],
                'link'=>$_POST['grid1_'.$index.'_link'][$key],
            );
        }
        $arr = array(
            'type'  => 'grid1',
            'grid_column_type'  => 'full',
            'items' => $items,
        );
        return $arr;
    }
    public function generate_html_field($row_num,$value)
    {
        ...
        $type_field = '<input type="hidden" name="post_builder_type[]" value="grid1">';
        ...
        foreach ($grids as $grid){
            ...
            $grid_fields .= '<td><input class="input-hidden-image" type="hidden" name="grid1_'.$row_num.'_image[]" value="'.$grid['image'].'">';
            ...
            $grid_fields .= '<td><select name="grid1_'.$row_num.'_link_type[]"><option value="-1">هیچ کدام</option>';
            ...
            $grid_fields .= '<td><input name="grid1_'.$row_num.'_link[]" value="'.$grid['link'].'"></td>';
            $grid_fields .= '<tr>';
        }
