...
class APPCHAR_WC_API_Orders extends APPCHAR_WC_API_Resource {
	...
	public function register_routes( $routes ) {
		...
		$routes[ $this->base ] = array(
			array( array( $this, 'get_orders' ),     APPCHAR_WC_API_Server::READABLE ),
			array( array( $this, 'create_order' ),   APPCHAR_WC_API_Server::CREATABLE | APPCHAR_WC_API_Server::ACCEPT_DATA ),
		);

		# GET /orders/count
		$routes[ $this->base . '/count' ] = array(
			array( array( $this, 'get_orders_count' ), APPCHAR_WC_API_Server::READABLE ),
		);

		# GET /orders/statuses
		$routes[ $this->base . '/statuses' ] = array(
			array( array( $this, 'get_order_statuses' ), APPCHAR_WC_API_Server::READABLE ),
		);

		# GET|PUT|DELETE /orders/<id>
		$routes[ $this->base . '/(?P<id>\d+)' ] = array(
			array( array( $this, 'get_order' ),  APPCHAR_WC_API_Server::READABLE ),
			array( array( $this, 'edit_order' ), APPCHAR_WC_API_Server::EDITABLE | APPCHAR_WC_API_Server::ACCEPT_DATA ),
			array( array( $this, 'delete_order' ), APPCHAR_WC_API_Server::DELETABLE ),
		);

		# GET|POST /orders/<id>/notes
		$routes[ $this->base . '/(?P<order_id>\d+)/notes' ] = array(
			array( array( $this, 'get_order_notes' ), APPCHAR_WC_API_Server::READABLE ),
			array( array( $this, 'create_order_note' ), APPCHAR_WC_API_Server::CREATABLE | APPCHAR_WC_API_Server::ACCEPT_DATA ),
		);

		# GET|PUT|DELETE /orders/<order_id>/notes/<id>
		$routes[ $this->base . '/(?P<order_id>\d+)/notes/(?P<id>\d+)' ] = array(
			array( array( $this, 'get_order_note' ), APPCHAR_WC_API_Server::READABLE ),
			array( array( $this, 'edit_order_note' ), APPCHAR_WC_API_Server::EDITABLE | APPCHAR_WC_API_Server::ACCEPT_DATA ),
			array( array( $this, 'delete_order_note' ), APPCHAR_WC_API_Server::DELETABLE ),
		);

		# GET|POST /orders/<order_id>/refunds
		$routes[ $this->base . '/(?P<order_id>\d+)/refunds' ] = array(
			array( array( $this, 'get_order_refunds' ), APPCHAR_WC_API_Server::READABLE ),
			array( array( $this, 'create_order_refund' ), APPCHAR_WC_API_Server::CREATABLE | APPCHAR_WC_API_Server::ACCEPT_DATA ),
		);

		# GET|PUT|DELETE /orders/<order_id>/refunds/<id>
		$routes[ $this->base . '/(?P<order_id>\d+)/refunds/(?P<id>\d+)' ] = array(
			array( array( $this, 'get_order_refund' ), APPCHAR_WC_API_Server::READABLE ),
			array( array( $this, 'edit_order_refund' ), APPCHAR_WC_API_Server::EDITABLE | APPCHAR_WC_API_Server::ACCEPT_DATA ),
			array( array( $this, 'delete_order_refund' ), APPCHAR_WC_API_Server::DELETABLE ),
		);
	...
	public function get_order( $id, $fields = null, $filter = array() ) {
		...
		foreach ( $order->get_items() as $item_id => $item ) {
			...
			$line_item = array(
				...
				'sku'          => $product_sku,
				'meta'         => $item_meta,
				'download_files' => $this->get_item_downloads($order, $item),
			);
	...
	public function get_orders_count( $status = null, $filter = array() ) {
		...
		try {
			/*if ( ! current_user_can( 'read_private_shop_orders' ) ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_api_user_cannot_read_orders_count', __( 'You do not have permission to read the orders count', 'woocommerce' ), 401 );
			}*/
	...
	public function create_order( $data ) {
		...
		try {
			if ( ! isset( $data['order'] ) ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_api_missing_order_data', sprintf( __( 'No %1$s data specified to create %1$s', 'woocommerce' ), 'order' ), 400 );
			}
			...
			/*if ( ! current_user_can( 'publish_shop_orders' ) ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_api_user_cannot_create_order', __( 'You do not have permission to create orders', 'woocommerce' ), 401 );
			}*/
			...
			if ( ! empty( $data['customer_id'] ) ) {
				...
				if ( false === get_user_by( 'id', $data['customer_id'] ) ) {
					throw new APPCHAR_WC_API_Exception( 'woocommerce_api_invalid_customer_id', __( 'Customer ID is invalid', 'woocommerce' ), 400 );
				}
			...
			if ( is_wp_error( $order ) ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_api_cannot_create_order', sprintf( __( 'Cannot create order: %s', 'woocommerce' ), implode( ', ', $order->get_error_messages() ) ), 400 );
			}
			...
			if ( isset( $data['payment_details'] ) && is_array( $data['payment_details'] ) ) {
				...
				if ( empty( $data['payment_details']['method_id'] ) || empty( $data['payment_details']['method_title'] ) ) {
					throw new APPCHAR_WC_API_Exception( 'woocommerce_invalid_payment_details', __( 'Payment method ID and title are required', 'woocommerce' ), 400 );
				}
			...
			if ( isset( $data['currency'] ) ) {
				...
				if ( ! array_key_exists( $data['currency'], get_woocommerce_currencies() ) ) {
					throw new APPCHAR_WC_API_Exception( 'woocommerce_invalid_order_currency', __( 'Provided order currency is invalid', 'woocommerce'), 400 );
				}
	 ...
	 *
	 * Requires a separate function for classes that extend APPCHAR_WC_API_Orders.
	 *
	 * @since 2.3
	 * @param $args array
	 * @return WC_Order
	...
	public function edit_order( $id, $data ) {
		try {
			if ( ! isset( $data['order'] ) ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_api_missing_order_data', sprintf( __( 'No %1$s data specified to edit %1$s', 'woocommerce' ), 'order' ), 400 );
			}
			...
			if ( empty( $order ) ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_api_invalid_order_id', __( 'Order ID is invalid', 'woocommerce' ), 400 );
			}
	 ...
	 * @throws APPCHAR_WC_API_Exception if item ID is not associated with order
	 */
	protected function set_item( $order, $item_type, $item, $action ) {
		...
		if ( 'update' === $action ) {
			...
			if ( is_null( $result ) ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_invalid_item_id', __( 'Order item ID provided is not associated with order', 'woocommerce' ), 400 );
			}
		}
	 ...
	 * @throws APPCHAR_WC_API_Exception invalid data, server error
	 */
	protected function set_line_item( $order, $item, $action ) {
		...
		if ( ! isset( $item['product_id'] ) && ! isset( $item['sku'] ) ) {
			throw new APPCHAR_WC_API_Exception( 'woocommerce_api_invalid_product_id', __( 'Product ID or SKU is required', 'woocommerce' ), 400 );
		}
		...
		if ( 'update' === $action ) {
			...
			if ( $item['product_id'] != $item_product_id && $item['product_id'] != $item_variation_id ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_api_invalid_product_id', __( 'Product ID provided does not match this line item', 'woocommerce' ), 400 );
			}
		}
		...
		if ( isset( $item['variations'] ) && is_array( $item['variations'] ) ) {
			foreach ( $item['variations'] as $key => $value ) {
				if ( ! $key || ! $value ) {
					throw new APPCHAR_WC_API_Exception( 'woocommerce_api_invalid_product_variation', __( 'The product variation is invalid', 'woocommerce' ), 400 );
				}
			}
		...
		if ( ! is_object( $product ) ) {
			throw new APPCHAR_WC_API_Exception( 'woocommerce_api_invalid_product', __( 'Product is invalid', 'woocommerce' ), 400 );
		}

		// quantity must be positive float
		if ( isset( $item['quantity'] ) && floatval( $item['quantity'] ) <= 0 ) {
			throw new APPCHAR_WC_API_Exception( 'woocommerce_api_invalid_product_quantity', __( 'Product quantity must be a positive float', 'woocommerce' ), 400 );
		}

		// quantity is required when creating
		if ( $creating && ! isset( $item['quantity'] ) ) {
			throw new APPCHAR_WC_API_Exception( 'woocommerce_api_invalid_product_quantity', __( 'Product quantity is required', 'woocommerce' ), 400 );
		}
		...
		if ( $creating ) {
			...
			if ( ! $item_id ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_cannot_create_line_item', __( 'Cannot create line item, try again', 'woocommerce' ), 500 );
			}
			...
			if ( ! $item_id ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_cannot_update_line_item', __( 'Cannot update line item, try again', 'woocommerce' ), 500 );
			}
		}
	}
	...
	protected function set_shipping( $order, $shipping, $action ) {
		...
		if ( isset( $shipping['total'] ) && floatval( $shipping['total'] ) < 0 ) {
			throw new APPCHAR_WC_API_Exception( 'woocommerce_invalid_shipping_total', __( 'Shipping total must be a positive amount', 'woocommerce' ), 400 );
		}
		...
		if ( 'create' === $action ) {
			...
			if ( ! isset( $shipping['method_id'] ) ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_invalid_shipping_item', __( 'Shipping method ID is required', 'woocommerce' ), 400 );
			}
			...
			if ( ! $shipping_id ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_cannot_create_shipping', __( 'Cannot create shipping method, try again', 'woocommerce' ), 500 );
			}
			...
			if ( ! $shipping_id ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_cannot_update_shipping', __( 'Cannot update shipping method, try again', 'woocommerce' ), 500 );
			}
		}
	}
	...
	protected function set_fee( $order, $fee, $action ) {
		...
		if ( 'create' === $action ) {
			...
			if ( ! isset( $fee['title'] ) ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_invalid_fee_item', __( 'Fee title is required', 'woocommerce' ), 400 );
			}
			...
			if ( isset( $fee['taxable'] ) && $fee['taxable'] ) {
				...
				if ( ! isset( $fee['tax_class'] ) ) {
					throw new APPCHAR_WC_API_Exception( 'woocommerce_invalid_fee_item', __( 'Fee tax class is required when fee is taxable', 'woocommerce' ), 400 );
				}
			...
			if ( ! $fee_id ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_cannot_create_fee', __( 'Cannot create fee, try again', 'woocommerce' ), 500 );
			}
			...
			if ( ! $fee_id ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_cannot_update_fee', __( 'Cannot update fee, try again', 'woocommerce' ), 500 );
			}
		}
	}
	...
	protected function set_coupon( $order, $coupon, $action ) {
		...
		if ( isset( $coupon['amount'] ) && floatval( $coupon['amount'] ) < 0 ) {
			throw new APPCHAR_WC_API_Exception( 'woocommerce_invalid_coupon_total', __( 'Coupon discount total must be a positive amount', 'woocommerce' ), 400 );
		}
		...
		if ( 'create' === $action ) {
			...
			if ( empty( $coupon['code'] ) ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_invalid_coupon_coupon', __( 'Coupon code is required', 'woocommerce' ), 400 );
			}

			$coupon_id = $order->add_coupon( $coupon['code'], isset( $coupon['amount'] ) ? floatval( $coupon['amount'] ) : 0 );

			if ( ! $coupon_id ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_cannot_create_order_coupon', __( 'Cannot create coupon, try again', 'woocommerce' ), 500 );
			}
			...
			if ( ! $coupon_id ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_cannot_update_order_coupon', __( 'Cannot update coupon, try again', 'woocommerce' ), 500 );
			}
		}
	}
	...
	public function get_order_note( $order_id, $id, $fields = null ) {
		try {
			...
			if ( empty( $id ) ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_api_invalid_order_note_id', __( 'Invalid order note ID', 'woocommerce' ), 400 );
			}

			$note = get_comment( $id );

			if ( is_null( $note ) ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_api_invalid_order_note_id', __( 'An order note with the provided ID could not be found', 'woocommerce' ), 404 );
			}
	...
	public function create_order_note( $order_id, $data ) {
		try {
			if ( ! isset( $data['order_note'] ) ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_api_missing_order_note_data', sprintf( __( 'No %1$s data specified to create %1$s', 'woocommerce' ), 'order_note' ), 400 );
			}
			...
			// permission check
			/*if ( ! current_user_can( 'publish_shop_orders' ) ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_api_user_cannot_create_order_note', __( 'You do not have permission to create order notes', 'woocommerce' ), 401 );
			}*/
			...
			if ( ! isset( $data['note'] ) ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_api_invalid_order_note', __( 'Order note is required', 'woocommerce' ), 400 );
			}
			...
			if ( ! $note_id ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_api_cannot_create_order_note', __( 'Cannot create order note, please try again', 'woocommerce' ), 500 );
			}
	...
	public function edit_order_note( $order_id, $id, $data ) {
		try {
			if ( ! isset( $data['order_note'] ) ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_api_missing_order_note_data', sprintf( __( 'No %1$s data specified to edit %1$s', 'woocommerce' ), 'order_note' ), 400 );
			}
			...
			if ( $note->comment_post_ID != $order->get_id() ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_api_invalid_order_note_id', __( 'The order note ID provided is not associated with the order', 'woocommerce' ), 400 );
			}
	...
	public function delete_order_note( $order_id, $id ) {
		try {
			...
			if ( ! $result ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_api_cannot_delete_order_note', __( 'This order note cannot be deleted', 'woocommerce' ), 500 );
			}
	...
	public function get_order_refund( $order_id, $id, $fields = null ) {
		try {
			...
			if ( empty( $id ) ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_api_invalid_order_refund_id', __( 'Invalid order refund ID', 'woocommerce' ), 400 );
			}
			...
			if ( ! $refund ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_api_invalid_order_refund_id', __( 'An order refund with the provided ID could not be found', 'woocommerce' ), 404 );
			}
	...
	public function create_order_refund( $order_id, $data, $api_refund = true ) {
		try {
			if ( ! isset( $data['order_refund'] ) ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_api_missing_order_refund_data', sprintf( __( 'No %1$s data specified to create %1$s', 'woocommerce' ), 'order_refund' ), 400 );
			}
			...
			// Permission check
			/*if ( ! current_user_can( 'publish_shop_orders' ) ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_api_user_cannot_create_order_refund', __( 'You do not have permission to create order refunds', 'woocommerce' ), 401 );
			}*/
			...
			if ( ! isset( $data['amount'] ) ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_api_invalid_order_refund', __( 'Refund amount is required', 'woocommerce' ), 400 );
			} elseif ( 0 > $data['amount'] ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_api_invalid_order_refund', __( 'Refund amount must be positive', 'woocommerce' ), 400 );
			}
			...
			if ( ! $refund ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_api_cannot_create_order_refund', __( 'Cannot create order refund, please try again', 'woocommerce' ), 500 );
			}
			...
			if ( $api_refund ) {
				...
				if ( isset( $payment_gateways[ $order->payment_method ] ) && $payment_gateways[ $order->payment_method ]->supports( 'refunds' ) ) {
					...
					if ( is_wp_error( $result ) ) {
						...
						throw new APPCHAR_WC_API_Exception( 'woocommerce_api_create_order_refund_api_failed', __( 'An error occurred while attempting to create the refund using the payment gateway API', 'woocommerce' ), 500 );
					}
				}
			}
	...
	public function edit_order_refund( $order_id, $id, $data ) {
		try {
			if ( ! isset( $data['order_refund'] ) ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_api_missing_order_refund_data', sprintf( __( 'No %1$s data specified to edit %1$s', 'woocommerce' ), 'order_refund' ), 400 );
			}
			...
			if ( $refund->post_parent != $order_id ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_api_invalid_order_refund_id', __( 'The order refund ID provided is not associated with the order', 'woocommerce' ), 400 );
			}
	 ...
	 * APPCHAR_WC_API_Orders->create_order() and APPCHAR_WC_API_Orders->edit_order()
	 *
	 * @since 2.4.0
	 * @param array $data
	 * @return array
	...
	public function bulk( $data ) {
		...
		try {
			if ( ! isset( $data['orders'] ) ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_api_missing_orders_data', sprintf( __( 'No %1$s data specified to create/edit %1$s', 'woocommerce' ), 'orders' ), 400 );
			}
			...
			if ( count( $data ) > $limit ) {
				throw new APPCHAR_WC_API_Exception( 'woocommerce_api_orders_request_entity_too_large', sprintf( __( 'Unable to accept more than %s items for this request', 'woocommerce' ), $limit ), 413 );
			}
	...
	public function get_item_downloads($order, $item) {
		$product = $order->get_product_from_item($item);
		$links          = array();
